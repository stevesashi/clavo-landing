...
const PROOF_NUMBERS = [
  { value: "10,000+", label: "Candidates Screened", color: "#fbbf24" },
  { value: "1,200+",  label: "Recruiter Hours Saved", color: "#6ee7b7" },
  { value: "92%",     label: "Match Accuracy",  color: "#a78bfa" },
  { value: "50+",     label: "Teams Across GCC & APAC", color: "#67e8f9" },
];
...
const CASE_STUDY = {
  ...
  results: {
    ...
    stats: [
      ...
      { value: "AED 68k",   label: "Saved in first month", color: "#fbbf24" },
    ...
    >
      ...
      <div className="flex gap-0.5" aria-label={`${stars} stars`}>
        {Array.from({ length: stars }).map((_, i) => (
          <Star key={i} size={12} fill="#fbbf24" className="text-amber-400" aria-hidden="true" />
        ))}
      </div>
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        <div className="absolute left-0 top-1/3 h-80 w-80 rounded-full blur-3xl"
          style={{ background: "rgba(110,231,183,0.04)" }} />
        <div className="absolute right-0 bottom-1/3 h-80 w-80 rounded-full blur-3xl"
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        >
          ...
          <p className="mx-auto mt-4 max-w-lg text-base leading-relaxed text-white/38">
            From solo recruiters to agency directors across GCC & APAC — here's what they say after switching.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          {TESTIMONIALS.map((t, i) => (
            <TestimonialCard key={t.author} {...t} delay={0.08 + i * 0.12} />
          ))}
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-0 md:grid-cols-3">
            ...
            <div className="border-b border-white/[0.06] p-8 md:border-b-0 md:border-r">
              <div className="mb-3 flex items-center gap-2">
                <div className="h-7 w-7 rounded-lg bg-amber-400/15 flex items-center justify-center" aria-hidden="true">
                  <Users size={14} className="text-amber-400" />
                </div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-amber-400/70">
              ...
              <ol className="flex flex-col gap-2.5" aria-label="Steps Clavo AI took">
                {CASE_STUDY.process.steps.map((step, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-amber-400/15 text-[9px] font-bold text-amber-400">
                      {i + 1}
                    </span>
